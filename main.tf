...
locals {
  ...
  # API output
  authorizer_id        = module.api.setup.authorizer_id
...
module "method" {
  ...
  uri         = format("arn:aws:apigateway:${local.region}:lambda:path/2015-03-31/functions/%s/invocations", module.lambda[each.value.lambda].lambda_function_arn)

  authorizer_id = try(local.authorizer_id[each.value.authorizer], null)
